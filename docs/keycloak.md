...
- chiudi le API con il flag sulla autenticazione
- crea un APP ID --> SAML
- configurazione con SAML
- crea una Application sotto APP ID per avere il json di configurazione

https://cloud.ibm.com/docs/appid?topic=appid-custom-identity

openssl genpkey -algorithm RSA -out private_openmed-app.pem -pkeyopt rsa_keygen_bits:2048
openssl rsa -pubout -in private_openmed-app.pem -out public_openmed-app.pem